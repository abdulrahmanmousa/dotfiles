...
-- Allow selecting and copying diagnostic messages
function Show_diagnostics_in_float()
  local opts = { focusable = true, close_events = { 'BufLeave', 'CursorMoved', 'InsertEnter', 'FocusLost' } }
  vim.diagnostic.open_float(nil, opts)
end

-- Map a key to open diagnostic messages in an interactive floating window
vim.api.nvim_set_keymap('n', '<leader>fe', '<cmd>lua Show_diagnostics_in_float()<CR>', { noremap = true, silent = true })
...
-- Minimal error flag with hover support
vim.diagnostic.config {
  ...
  underline = {
    ...
    highlight = 'DiagnosticErrorLine',
  },
  update_in_insert = false, -- Avoid updates during insert mode
  severity_sort = true, -- Sort diagnostics by severity
}

-- Custom symbols for errors, warnings, etc.
local signs = { Error = ' ', Warn = ' ', Hint = ' ', Info = ' ' }
for type, icon in pairs(signs) do
  local hl = 'DiagnosticSign' .. type
  vim.fn.sign_define(hl, { text = icon, texthl = hl, numhl = '' })
...
-- Show diagnostics in a floating popup on hover
vim.api.nvim_create_autocmd('CursorHold', {
  callback = function()
    vim.diagnostic.open_float(nil, { focusable = false })
  end,
...
vim.keymap.set('n', '<leader>ic', '<cmd>TSToolsRemoveUnused<CR>')
vim.keymap.set('n', '<leader>ia', '<cmd>TSToolsAddMissingImports<CR>')

--
--
-- show file history keymaps
vim.keymap.set('n', '<leader>fh', function()
  require('telescope').extensions.file_history.history()
end, { desc = 'Show [F]ile [H]istory' })
--
-- -- Neotree keymaps
vim.keymap.set('n', '<leader>r', function()
  vim.cmd 'Neotree' -- Move the netrw window to the left, making it full screen
end, { noremap = true, silent = true })

vim.keymap.set('n', '<leader>e', ':Neotree toggle<CR>', { noremap = true, silent = true })

-- move to previous buffer mapping ctrl + ^
...
vim.keymap.set('n', '<A-d>', ':m .+1<CR>==', { noremap = true, silent = true })
vim.keymap.set('n', '<A-u>', ':m .-2<CR>==', { noremap = true, silent = true })
...
require('lazy').setup({
  ...
  { -- Highlight, edit, and navigate code
    ...
    build = ':TSUpdate',
    opts = {
      ensure_installed = { 'bash', 'c', 'html', 'lua', 'markdown', 'vim', 'vimdoc', 'typescript', 'javascript', 'python' },
